...
main() {
	...
	docker run --rm -it -v ${realTargetPath}:${mappedTargetPath} "$projectName" \
		cp -v \
		/root/sources/openwrt/openwrt-imagebuilder-19.07.6-brcm47xx-legacy.Linux-x86_64/bin/targets/brcm47xx/legacy/openwrt-19.07.6-brcm47xx-legacy-asus-wl-500gp-v2-squashfs.trx \
		$mappedTargetPath/
}
